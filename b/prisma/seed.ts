...
async function main() {
  ...
  console.log(
    `  ✅ Photographer profile ready (id=${photographer.id}) linked to state=${algiers?.name}`,
  );
  ...
  console.log(
    `  ✅ Conversation created (id=${conversation.id}) with ${conversation.messages.length} messages`,
  );
