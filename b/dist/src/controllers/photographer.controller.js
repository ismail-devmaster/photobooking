...
exports.listPhotographersByState = listPhotographersByState;
exports.getPhotographer = getPhotographer;
exports.getPhotographerStats = getPhotographerStats;
const photographerService = __importStar(require("../services/photographer.service"));
 ...
 */
async function listPhotographersByState(req, res) {
    try {
        // Merge path param into query for validation
        const input = { ...req.query, stateId: req.params.stateId };
        const parsed = profile_filter_schemas_1.photographerListQuery.safeParse(input);
        if (!parsed.success) {
            return res.status(400).json({ error: 'Invalid query', issues: parsed.error.issues });
        }
        const q = parsed.data;
 ...
 */
async function getPhotographerStats(req, res) {
    try {
        const { id } = req.params;
